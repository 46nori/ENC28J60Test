...
//	enc28j60PhyWrite(PHLCON, 0x0AA2);
	enc28j60PhyWrite(PHLCON, 0x0742); // LED A: Tx/Rx Activity / LED B: Link
	...
	
	{
...
#if defined(ENC28J60_DUPLEX_HALF)
		isFullDuplex = 0;
#elif defined(ENC28J60_DUPLEX_FULL)
		isFullDuplex = 1;
#else
		isFullDuplex = temp & PHCON1_PDPXMD;
#endif
		if (isFullDuplex)
			temp |= PHCON1_PDPXMD;
		else
		...
		if (isFullDuplex) {
			temp |= MACON3_FULDPX;
			rprintfProgStrM("FULL duplex\n");
		} else {
			temp &= ~MACON3_FULDPX;
			rprintfProgStrM("HALF duplex\n");
		}
		...
		enc28j60Write(MABBIPG, isFullDuplex ? 0x15 : 0x12);	
	}
...
//		rprintf("100BASE-T      \r\n");
	...
	rprintfProgStrM("MAC  : MACON1  MACON3  MACON4  MABBIPG MAC-Address\r\n");
	rprintfProgStrM("        0x");
	...
	rprintfProgStrM("    0x");
	rprintfu08(enc28j60Read(MABBIPG));
	rprintfProgStrM("   ");
	...
	rprintfProgStrM("PHY  : PHSTAT1 PHSTAT2 PHCON1 PHCON2\r\n");
	rprintfProgStrM("       0x");
	rprintfu16(enc28j60PhyRead(PHSTAT1));
	rprintfProgStrM("  0x");
	rprintfu16(enc28j60PhyRead(PHSTAT2));
	rprintfProgStrM("  0x");
	rprintfu16(enc28j60PhyRead(PHCON1));
	rprintfProgStrM(" 0x");
	rprintfu16(enc28j60PhyRead(PHCON2));
	rprintfCRLF();
